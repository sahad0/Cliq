  ...
  export default class OrgFlatlist extends PureComponent<Props> {
    ...
    renderItem:ListRenderItem<OrganisationType> = ({ item }) => (
      ...
      <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
          <View style={{flexDirection:'row',alignItems:'center'}}>
              ...
              <Text style={{fontFamily:'ZohoRegular',fontSize:this.props.height*0.02,color:item.isDefault ? 'white' :'#2677C3'}}>{item.name}</Text>
          </View>
          {
            item.isDefault && (
    ...
    HeaderComp = ()=>(
      <View style={{margin:this.props.height*0.04,flexDirection:'row',alignItems:'center'}}>
          ...
          <Text style={{fontFamily:'ZohoRegular',fontSize:this.props.height*0.025,color:'black',marginLeft:this.props.width*0.03}}>Organizations</Text>
      </View>
    )
